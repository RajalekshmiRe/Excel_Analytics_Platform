...
// ============================================================
// ✅ FIXED: Upload History Component
// ============================================================
function UploadHistory({ theme, currentUser }) {
  const [uploads, setUploads] = useState([]);
  const [loading, setLoading] = useState(true);
    ...
    const fetchUploads = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const uploadsArray = data.uploads || data || [];
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        ...
        // ✅ FIX: Use correct API endpoint
        const response = await fetch(
          `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/analysis/analytics`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
        ...
        if (data.success && data.data) {
          setAnalyticsData(data.data);
        } else if (data.totalUploads !== undefined) {
          // Direct format
  ...
  return (
    <div className={`p-8 lg:p-12 min-h-screen ${bgColor}`}>
      ...
      {analyticsData ? (
                ...
                {analyticsData.typeData && analyticsData.typeData.length > 0 ? (
                  analyticsData.typeData.map((item, idx) => (
                    <div
                      ...
                      className="flex justify-between items-center"
                    >
                      <span className="font-semibold">{item.type || item._id}</span>
                      <span className="text-[#bc4e9c] font-bold">
                        {item.count} files
                      </span>
                    </div>
              ...
              <div className="space-y-4">
                ...
                analyticsData.recentUploads.length > 0 ? (
                  analyticsData.recentUploads.slice(0, 5).map((upload, idx) => (
                    ...
                    >
                      <span className="font-semibold truncate max-w-[200px]">
                        {upload.fileName || upload.filename || upload.originalName}
                      </span>
                      <span className="text-sm text-gray-600 dark:text-gray-400">
                        {new Date(upload.uploadDate || upload.createdAt).toLocaleDateString()}
                      </span>
                    </div>
                  ))
                ) : (
        ...
        >
          <p className="text-gray-600 dark:text-gray-400 text-lg">
            No analytics data available yet. Start uploading files to see your analytics!
          </p>
        </div>
      )}
    </div>
...
// ============================================================
// Main Dashboard Layout (No changes needed)
// ============================================================
export default function DashboardLayout() {
  const navigate = useNavigate();
