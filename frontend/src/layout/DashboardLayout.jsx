  ...
  return (
    <div className={`p-8 lg:p-12 min-h-screen ${bgColor}`}>
      ...
      {loading ? (
        ...
        <p className="text-gray-600 dark:text-gray-400 text-lg">No uploads yet.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {uploads.map((file, idx) => (
            <div
              key={idx}
              className={`p-8 rounded-2xl shadow-xl transition-all hover:shadow-2xl hover:scale-105 ${cardGradient} border-2 border-blue-300 dark:border-blue-600`}
            >
              <h3 className="font-bold text-lg break-all mb-4">{file.originalName}</h3>
              ...
              <p className="text-gray-600 dark:text-gray-400 text-base">
                Status: <span className={`font-bold ${file.status === 'processed' ? 'text-green-600' : 'text-yellow-600'}`}>
...
function Analytics({ theme, currentUser }) {
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        ...
        const response = await fetch('http://localhost:5000/api/analysis/analytics', {
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  if (loading) {
    return (
      <div className={`p-8 lg:p-12 min-h-screen ${bgColor} flex items-center justify-center`}>
  ...
  if (error) {
    return (
      <div className={`p-8 lg:p-12 min-h-screen ${bgColor}`}>
        ...
        <h2 className="text-3xl lg:text-4xl font-bold mb-10">Analytics Dashboard</h2>
        <div className={`p-10 rounded-2xl shadow-xl ${cardGradient} border-2 border-red-300 dark:border-red-600`}>
          <div className="text-center">
            ...
            <button
              ...
              className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-8 py-3 rounded-xl shadow-lg hover:shadow-xl hover:from-blue-600 hover:to-blue-700 transition-all font-bold"
            >
              Retry
            </button>
          </div>
      ...
      {analyticsData ? (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-10">
            <div className={`p-8 rounded-2xl shadow-xl ${cardGradient} border-2 border-blue-300 dark:border-blue-600`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className={`p-10 rounded-2xl shadow-xl ${cardGradient} border-2 border-blue-300 dark:border-blue-600`}>
        ...
        <div className={`p-10 rounded-2xl shadow-xl ${cardGradient} border-2 border-blue-300 dark:border-blue-600 text-center`}>
  ...
  return (
    <div className={`p-8 lg:p-12 min-h-screen ${bgColor}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        {/* Profile Info */}
        <div className={`p-10 rounded-2xl shadow-xl ${cardGradient} border-2 border-blue-300 dark:border-blue-600`}>
          <h3 className="text-2xl lg:text-3xl font-bold mb-8">Basic Information</h3>
          <div className="flex flex-col gap-8">
            <div>
              <label className="block text-base font-bold mb-3">Full Name</label>
              <input
                type="text"
                value={name}
        ...
        {/* Password & Theme */}
        <div className={`p-10 rounded-2xl shadow-xl ${cardGradient} border-2 border-blue-300 dark:border-blue-600`}>
          <h3 className="text-2xl lg:text-3xl font-bold mb-8">Security & Appearance</h3>
          <div className="flex flex-col gap-8">
            <div>
              <label className="block text-base font-bold mb-3">Change Password</label>
              <input
                type="password"
...
export default function DashboardLayout() {
  ...
  const renderActiveView = () => {
    switch (activeView) {
     ...
     case "settings":
  return <Settings currentUser={user} theme={theme} setTheme={setTheme} />;
      case "request-admin":
  ...
  return (
    ...
    >
      {/* Sidebar */}
      <aside
        className={`hidden lg:flex flex-col transition-all duration-300 shadow-2xl ${
          isSidebarOpen ? "w-72" : "w-24"
        } ${
          theme === "dark"
            ? "bg-gradient-to-b from-blue-900 to-blue-950 border-r border-blue-700"
            : "bg-gradient-to-b from-blue-100 to-blue-200 border-r border-blue-300"
        }`}
      >
        {/* User Section */}
        <div className="p-6 border-b border-blue-300 dark:border-blue-700">
          {isSidebarOpen ? (
            <div className="flex items-center gap-4">
              <div className="relative">
                ...
                <div className="absolute -bottom-1 -right-1 h-4 w-4 bg-green-500 border-2 border-blue-900 rounded-full"></div>
              </div>
        ...
        <button
          ...
          className="mx-4 mt-4 p-3 text-gray-700 dark:text-gray-200 hover:bg-blue-500/30 dark:hover:bg-blue-400/30 rounded-xl transition-all font-bold"
        >
            ...
            <button
              ...
              className={`flex items-center gap-4 px-5 py-4 rounded-xl w-full transition-all font-semibold text-base ${
                activeView === id
                  ...
                  : "text-blue-900 dark:text-blue-100 hover:bg-blue-500/20 dark:hover:bg-blue-400/20"
              }`}
            >
        ...
        <div className="p-4 border-t border-blue-300 dark:border-blue-700">
          <button
            ...
            className="flex items-center gap-4 px-5 py-4 rounded-xl text-red-600 hover:bg-red-500/20 w-full transition-all font-semibold text-base"
          >
            <LogOut size={22} />
