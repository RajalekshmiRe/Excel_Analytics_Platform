...
//     console.log("✅ Raw analytics response:", res.data);
...
//         reportsTrend: 0
//       },
//       uploadTrend: Array.isArray(analyticsData.uploadTrend) ? analyticsData.uploadTrend : [],
//       fileTypes: Array.isArray(analyticsData.fileTypes) ? analyticsData.fileTypes : [],
//       storageUsage: Array.isArray(analyticsData.storageUsage) ? analyticsData.storageUsage : [],
//       recentActivity: Array.isArray(analyticsData.recentActivity) ? analyticsData.recentActivity : [],
//       topFiles: Array.isArray(analyticsData.topFiles) ? analyticsData.topFiles : []
//     };
...
//     if (timeRange !== 'all') {
//       toast.success(`Analytics filtered to: ${
...
//     // Set up polling interval - SILENT refresh (no loading state)
...
//         {/* Export Section - ✅ ENHANCED WITH WORKING FUNCTIONALITY */}
//         <div className={`${cardBg} rounded-2xl shadow-lg border ${borderColor} p-6 mt-8`}>
//           <div className="flex items-center justify-between">
//             <div>
//               <h3 className={`text-xl font-bold ${textColor} mb-2`}>Export Analytics Report</h3>
...
export default function Analytics() {
  ...
  const [refreshing, setRefreshing] = useState(false);
  const [exporting, setExporting] = useState(false);
  const [lastUpdate, setLastUpdate] = useState(new Date()); // ✅ FIXED: Added missing state
  ...
  const fetchAnalytics = async (silent = false) => {
    ...
    try {
      ...
      // Only show toast for manual time range changes
      if (!silent && timeRange !== 'all') {
        toast.success(`Analytics filtered to: ${
          timeRange === '7days' ? 'Last 7 Days' : 
          timeRange === '30days' ? 'Last 30 Days' : 
          'Last 90 Days'
  ...
  // ✅ POLLING + TIME RANGE: Auto-refresh with real-time updates
  useEffect(() => {
    if (!currentUser?.id) return;
    ...
    // Set up polling interval - SILENT refresh (no loading spinner)
      ...
      <div className={`min-h-screen ${bgColor} flex items-center justify-center p-8`}>
        ...
        <div className={`${cardBg} rounded-3xl shadow-2xl p-8 border-2 border-[#bc4e9c] max-w-md w-full`}>
          <div className="text-center">
            ...
            <button
              onClick={() => fetchAnalytics()}
              className="px-6 py-3 bg-gradient-to-r from-[#bc4e9c] to-[#f80759] text-white rounded-xl font-semibold hover:from-[#a0428a] hover:to-[#e0064f] transition-all shadow-lg flex items-center gap-2 mx-auto"
            >
              <RefreshCw className="w-5 h-5" />
              Try Again
            </button>
    ...
    return (
      <div className={`min-h-screen ${bgColor} flex items-center justify-center p-8`}>
        ...
        <div className={`${cardBg} rounded-3xl shadow-2xl p-12 border-2 border-[#bc4e9c] max-w-md w-full text-center`}>
          ...
          <h3 className={`text-xl font-bold ${textColor} mb-2`}>No Analytics Data Available</h3>
          <p className="text-gray-600 mb-6">Upload some files to start seeing your analytics</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-3">
              <button
                onClick={handleRefresh}
                disabled={refreshing}
                className={`px-4 py-2 rounded-xl border-2 border-[#bc4e9c] ${cardBg} ${textColor} font-semibold hover:bg-[#bc4e9c] hover:text-white transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed`}
              >
                <RefreshCw className={`w-5 h-5 ${refreshing ? 'animate-spin' : ''}`} />
                {refreshing ? 'Refreshing...' : 'Refresh'}
              </button>
