...
import { Upload, BarChart3, FileSpreadsheet, Brain, Handshake, FileText, Map, X, Download, PieChart, TrendingUp } from "lucide-react";
...
export default function DashboardHome({ currentUser, theme }) {
  const [file, setFile] = useState(null);
  const [fileData, setFileData] = useState(null);
  const [stats, setStats] = useState({ uploads: 0, storage: "0 MB", reports: 0, charts: 0 });
  const [activeModal, setActiveModal] = useState(null);
  const [chartConfig, setChartConfig] = useState({ type: 'bar', xAxis: '', yAxis: '' });
  const [mappingConfig, setMappingConfig] = useState({ sourceCol: '', targetCol: '', transformation: 'none' });
  ...
  const handleFileUpload = async (e) => {
    ...
    const fileExtension = selected.name.substring(selected.name.lastIndexOf('.')).toLowerCase();
    ...
    try {
      ...
      setStats((prev) => ({
        ...
        storage: `${(parseFloat(prev.storage) + selected.size / 1048576).toFixed(2)} MB`,
      }));
    ...
    if (fileExtension === '.csv') {
      ...
      reader.onload = (event) => {
        ...
        const rows = lines.map(line => {
          ...
          return matches.length > 0 ? matches : line.split(",").map(cell => cell.trim());
        });
  ...
  const generateChart = () => {
    ...
    const chartData = fileData.rows
      .map((row, idx) => {
        ...
        let numericValue = parseFloat(yValue?.toString().replace(/[^0-9.-]/g, ''));
    ...
    if (chartData.length === 0) {
      toast.error("No valid numeric data found for the selected Y-axis column!");
      return;
    }
    ...
    setActiveModal(null);
    toast.success(`Chart created successfully with ${chartData.length} data points!`);
  };
  ...
  const applyMapping = () => {
    ...
    if (numericTransformations.includes(mappingConfig.transformation)) {
      ...
      const testNumeric = parseFloat(firstValue?.toString().replace(/[^0-9.-]/g, ''));
      
      if (isNaN(testNumeric)) {
        toast.error(`Cannot apply numeric transformation "${mappingConfig.transformation}" to non-numeric column "${mappingConfig.sourceCol}"! Please select a numeric column or use text transformations.`);
        return;
      }
    }
    ...
    const newRows = fileData.rows.map(row => {
      ...
      switch (mappingConfig.transformation) {
        ...
        case 'lowercase':
          ...
          break;
        case 'double':
          const doubleVal = parseFloat(value?.toString().replace(/[^0-9.-]/g, ''));
          value = isNaN(doubleVal) ? value : (doubleVal * 2).toString();
          break;
        case 'half':
          const halfVal = parseFloat(value?.toString().replace(/[^0-9.-]/g, ''));
    ...
    setActiveModal(null);
    toast.success(`Data mapping applied successfully! ${mappingConfig.sourceCol} → ${mappingConfig.targetCol}`);
  };
  ...
  const cardGradient = theme === "dark" ? "bg-gradient-to-br from-blue-900 to-blue-800" : "bg-gradient-to-br from-blue-50 to-blue-100";
  const textColor = theme === "dark" ? "text-gray-100" : "text-gray-900";
  const bgColor = theme === "dark" ? "bg-gradient-to-b from-slate-900 via-blue-950 to-slate-900" : "bg-gradient-to-b from-blue-50 via-blue-100 to-blue-50";

  return (
    <div className={`w-full min-h-screen ${bgColor} transition-all`}>
      <Toaster position="top-center" />

      {/* Welcome Section */}
      <div className="px-8 lg:px-12 pt-12 pb-6">
        <h1 className="text-4xl lg:text-5xl font-bold flex items-center gap-3">
          Welcome, {currentUser.name}! <Handshake className="text-blue-500" size={40} />
        </h1>
        <p className="text-gray-600 dark:text-gray-400 mt-3 text-lg">Manage your data with ease and efficiency</p>
      </div>
      ...
      <div className="px-8 lg:px-12 py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {[
            { title: "Total Uploads", value: stats.uploads, icon: Upload },
            { title: "Storage Used", value: stats.storage, icon: FileSpreadsheet },
            { title: "Active Reports", value: stats.reports, icon: Brain },
            { title: "Charts Generated", value: stats.charts, icon: BarChart3 },
          ].map(({ title, value, icon: Icon }) => (
            <div
              key={title}
              className={`p-8 lg:p-10 rounded-3xl shadow-2xl transition-all duration-300 hover:shadow-3xl hover:scale-105 ${cardGradient} border-2 border-blue-300 dark:border-blue-600 group cursor-pointer`}
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <h3 className="text-sm lg:text-base font-bold text-blue-600 dark:text-blue-300 mb-3 uppercase tracking-wide">
                    {title}
                  </h3>
                  <p className={`text-3xl lg:text-4xl font-bold ${textColor}`}>{value}</p>
                </div>
                <div className="p-5 bg-blue-500/25 rounded-2xl group-hover:bg-blue-500/35 transition-all">
      ...
      <div className="px-8 lg:px-12 py-2">
        <hr className="border-t-2 border-blue-300 dark:border-blue-600" />
      </div>

      {/* Upload Section */}
      <div className="px-8 lg:px-12 py-10">
        <h2 className="text-2xl lg:text-3xl font-bold mb-8">Upload Your Data</h2>
        <div className={`p-12 lg:p-16 rounded-3xl shadow-2xl ${cardGradient} border-2 border-blue-300 dark:border-blue-600 transition-all`}>
          <div className="flex flex-col items-center justify-center gap-8 py-8">
            <div className="p-8 bg-blue-500/25 rounded-3xl group-hover:bg-blue-500/35 transition-all">
              <Upload className="h-16 w-16 text-blue-600 dark:text-blue-400" />
            </div>
            <div className="text-center">
              <h3 className="text-2xl lg:text-3xl font-bold mb-3">Upload CSV, XLS, or XLSX Files</h3>
              <p className="text-gray-600 dark:text-gray-400 text-lg">Drag and drop or click to select files</p>
            </div>
            <input 
              type="file" 
              accept=".xlsx,.xls,.csv" 
            ...
            <button 
              ...
              className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-12 py-4 rounded-2xl shadow-lg hover:shadow-2xl hover:from-blue-600 hover:to-blue-700 transition-all font-bold text-lg"
            >
              Choose File
            </button>
            {file && (
              <p className={`text-blue-600 dark:text-blue-300 font-bold text-lg break-all`}>
                ✓ {file.name}
              </p>
            )}
          </div>
          ...
          {/* File Preview */}
          {(fileData || mappedData) && (
            <div className="mt-10 bg-white dark:bg-blue-900/50 rounded-3xl p-8 border-2 border-blue-300 dark:border-blue-600 shadow-lg">
              <div className="flex justify-between items-center mb-6">
                <h3 className="font-bold text-2xl">{file.name} {mappedData && "(Mapped)"}</h3>
                <button 
                  onClick={closePreview} 
                  className="text-red-500 hover:text-red-700 text-base hover:underline font-bold"
                >
                  Close Preview
                </button>
              </div>
              <div ref={tableRef} className="max-h-96 overflow-x-auto overflow-y-auto rounded-2xl border-2 border-blue-300 dark:border-blue-600">
                <table className="w-full border-collapse text-base">
                  <thead className="bg-gradient-to-r from-blue-600 to-blue-700 text-white sticky top-0">
                    <tr>
                      {(mappedData || fileData).headers.map((header, idx) => (
                        <th key={idx} className="border border-blue-400 px-6 py-4 text-left font-bold">
                          {header}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {(mappedData || fileData).rows.map((row, rowIdx) => (
                      <tr key={rowIdx} className={`${rowIdx % 2 === 0 ? "bg-blue-50 dark:bg-blue-950" : "bg-white dark:bg-blue-900"} hover:bg-blue-100 dark:hover:bg-blue-800 transition`}>
                        {row.map((cell, cellIdx) => (
                          <td key={cellIdx} className="border border-blue-200 dark:border-blue-600 px-6 py-3">
                            {cell}
                          </td>
                        ))}
                      </tr>
...
{generatedChart && generatedChart.data && generatedChart.data.length > 0 && (
  <div className="mt-10 bg-white dark:bg-blue-900/50 rounded-3xl p-8 border-2 border-blue-300 dark:border-blue-600 shadow-lg">
    <div className="flex justify-between items-center mb-6">
      <div>
        <h3 className="font-bold text-2xl">Generated Chart</h3>
        <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
          {generatedChart.xAxisLabel} vs {generatedChart.yAxisLabel} ({generatedChart.data.length} points)
        </p>
      </div>
      <button 
    ...
    >
      {generatedChart.config.type === 'bar' && (
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={generatedChart.data}>
            ...
            <Tooltip />
            <Legend />
            <Bar dataKey="value" fill="#3b82f6" name={generatedChart.yAxisLabel} />
          </BarChart>
        </ResponsiveContainer>
      )}
      {generatedChart.config.type === 'line' && (
      ...
      <div className="px-8 lg:px-12 py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-10">
          {[
            { title: "Create Charts", icon: BarChart3, desc: "Generate stunning visual insights from your data", action: handleCreateChart },
            { title: "Data Mapping", icon: Map, desc: "Map and organize your data with precision", action: handleDataMapping },
            { title: "Export Reports", icon: FileText, desc: "Export detailed reports in multiple formats", action: handleExport },
          ].map(({ title, icon: Icon, desc, action }) => (
            <div
              key={title}
              className={`p-10 lg:p-12 rounded-3xl shadow-2xl flex flex-col items-center gap-6 transition-all duration-300 hover:shadow-3xl hover:scale-105 ${cardGradient} border-2 border-blue-300 dark:border-blue-600 group`}
            >
              <div className="p-6 bg-blue-500/25 group-hover:bg-blue-500/35 rounded-2xl transition-all">
                <Icon className="h-12 w-12 text-blue-600 dark:text-blue-400" />
              </div>
              <h3 className={`text-2xl lg:text-3xl font-bold text-center ${textColor}`}>{title}</h3>
              <p className="text-gray-600 dark:text-gray-400 text-center text-lg">{desc}</p>
              <button 
                onClick={action}
                className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-10 py-3 rounded-2xl shadow-lg hover:shadow-xl hover:from-blue-600 hover:to-blue-700 transition-all font-bold text-lg"
              >
                Explore
              </button>
            </div>
      ...
      {activeModal === 'chart' && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className={`${cardGradient} rounded-3xl p-8 max-w-md w-full shadow-2xl border-2 border-blue-300 dark:border-blue-600`}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-2xl font-bold">Create Chart</h3>
              <button onClick={() => setActiveModal(null)} className="text-gray-500 hover:text-gray-700 dark:text-gray-300 dark:hover:text-gray-100">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-bold mb-2 text-gray-700 dark:text-gray-200">Chart Type</label>
                <select 
                  value={chartConfig.type}
                  onChange={(e) => setChartConfig({...chartConfig, type: e.target.value})}
                  className="w-full p-3 rounded-xl border-2 border-blue-300 dark:border-blue-600 bg-white dark:bg-blue-900 text-gray-900 dark:text-gray-100"
                >
                  <option value="bar">Bar Chart</option>
                  <option value="line">Line Chart</option>
                  <option value="pie">Pie Chart</option>
              ...
              <div>
                <label className="block text-sm font-bold mb-2 text-gray-700 dark:text-gray-200">X-Axis Column (Labels)</label>
                <select 
                  value={chartConfig.xAxis}
                  onChange={(e) => setChartConfig({...chartConfig, xAxis: e.target.value})}
                  className="w-full p-3 rounded-xl border-2 border-blue-300 dark:border-blue-600 bg-white dark:bg-blue-900 text-gray-900 dark:text-gray-100"
              ...
              <div>
                <label className="block text-sm font-bold mb-2 text-gray-700 dark:text-gray-200">Y-Axis Column (Numeric Values)</label>
                <select 
                  value={chartConfig.yAxis}
                  onChange={(e) => setChartConfig({...chartConfig, yAxis: e.target.value})}
                  className="w-full p-3 rounded-xl border-2 border-blue-300 dark:border-blue-600 bg-white dark:bg-blue-900 text-gray-900 dark:text-gray-100"
              ...
              <button 
                ...
                className="w-full bg-gradient-to-r from-blue-500 to-blue-600 text-white px-6 py-3 rounded-xl shadow-lg hover:shadow-xl hover:from-blue-600 hover:to-blue-700 transition-all font-bold"
              >
                Generate Chart
              </button>
            </div>
      ...
      {activeModal === 'mapping' && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className={`${cardGradient} rounded-3xl p-8 max-w-md w-full shadow-2xl border-2 border-blue-300 dark:border-blue-600`}>
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-bold mb-2 text-gray-700 dark:text-gray-200">Source Column</label>
                <select 
                  value={mappingConfig.sourceCol}
                  onChange={(e) => setMappingConfig({...mappingConfig, sourceCol: e.target.value})}
                  className="w-full p-3 rounded-xl border-2 border-blue-300 dark:border-blue-600 bg-white dark:bg-blue-900 text-gray-900 dark:text-gray-100"
                >
                  <option value="">Select column...</option>
                  {fileData?.headers.map((header, idx) => (
              ...
              <div>
                <label className="block text-sm font-bold mb-2 text-gray-700 dark:text-gray-200">Target Column</label>
                <select 
                  value={mappingConfig.targetCol}
                  onChange={(e) => setMappingConfig({...mappingConfig, targetCol: e.target.value})}
                  className="w-full p-3 rounded-xl border-2 border-blue-300 dark:border-blue-600 bg-white dark:bg-blue-900 text-gray-900 dark:text-gray-100"
                >
                  <option value="">Select column...</option>
                  {fileData?.headers.map((header, idx) => (
              ...
              <div>
                <label className="block text-sm font-bold mb-2 text-gray-700 dark:text-gray-200">Transformation</label>
                <select 
                  value={mappingConfig.transformation}
                  onChange={(e) => setMappingConfig({...mappingConfig, transformation: e.target.value})}
                  className="w-full p-3 rounded-xl border-2 border-blue-300 dark:border-blue-600 bg-white dark:bg-blue-900 text-gray-900 dark:text-gray-100"
                >
